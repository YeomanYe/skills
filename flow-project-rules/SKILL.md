...
- `project-rules-design` 负责：入口结构、分域、层级、重复、混层、优先级、以及每个领域目录是否同时具备 `index.md` 和 `rules.md`
- 技术栈 skill 负责：该栈应该被写入规范的工程约束
- 用户额外指定的 skill：按其自述职责审视

「某个栈应该进入哪个规则域、覆盖哪些关键点」这一对照清单，统一读 `project-rules-design/references/stack-checklist.md`。本 skill 不在正文里自己维护一份短名单，避免两侧漂移。

若 `stack-checklist.md` 没覆盖当前识别到的某个栈，按 checklist 末尾的"未覆盖栈的处理"规则走：不硬套最接近条目，在最终报告中显式保留缺口。
...
### 🟡 中 ROI 视情况派
- **Step 7 落地修改**（≥ 10 文件迁移 / ≥ 30 处引用更新）：Claude 把每个 mv + 每处引用更新写进 SPEC（含 voice 保留约束），Codex 实施，Claude 验收引用是否真解析 + 风格是否漂移
- 派工前提：迁移规模真大（小规模 < 10 文件时 SPEC 撰写成本 ≈ 直接做）

### 🔴 低 / 负 ROI 不建议派
- **Step 1-5 评估阶段**：识别栈 / 匹配 skill / 读取规范 / 联合评估都是 judgment-heavy，Codex 起新进程拿不到完整评估框架
- **Step 6 改进计划**：决策输出
- **Step 7 小规模迁移**（< 10 文件）：SPEC 撰写成本 > 节省
- **Step 8 最终报告**：依赖整个评估上下文
