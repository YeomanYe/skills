---
name: post-to-v2ex
description: Use when 用户要在 V2EX 发主题/帖子，或要求 "在 v2ex 发个 share / 发到分享创造 / post to v2ex / 发个 v2ex 帖子"。通过 playwriter 控制用户已登录的 Chrome，定位发帖节点，从当前项目提炼标题+正文+配图，切到 Markdown 语法，调用 V2EX 自带的 previewTopicContent() 预览并截图给用户确认，默认停在预览页让用户手动点"创建主题"，除非用户显式授权直接提交。
---

# Post to V2EX

通过 playwriter 控制用户当前已登录的 Chrome，在 V2EX 指定节点发主题贴。素材（标题/正文/配图链接）从当前项目仓库提炼，配图通过 GitHub raw 链接嵌入，发布前必须截图预览给用户确认。

## When to Use

- 用户要求"在 V2EX 发个帖子"、"发到分享创造"、"post to v2ex"、"v2ex 发个 share / create / qna"
- 要为当前项目 / 产品 / 文章在 V2EX 做推广
- 用户已经在 Chrome 里登录了 V2EX

不要用于：

- 通过 V2EX API 编程发帖 — 那是另一回事
- 用户没登录的情况下发帖 — 跳出来让用户先登录
- 发推 / Twitter / X → 用 `post-to-twitter`
- Product Hunt / 其它平台 → 各自有自己的 skill
...
如果 playwriter 不可用，告知用户安装 playwriter 浏览器扩展并激活，**不要**回退到 Playwright headless 浏览器（那是另一个 Chrome 实例，没有用户登录态）。

## Why playwriter, not standard Playwright MCP

playwriter 直连用户当前 Chrome，保留 V2EX 登录 cookie。Playwright MCP / Chrome DevTools MCP 都会开新会话，发帖时跳到登录页。
...
- **标题**：项目名 + 一句价值（中文，控制在 50 字内，V2EX 标题宽容但短的更易点）
...
- README 里的 Firefox/Edge/Chrome 商店链接可能还是 placeholder（`#` 或 `YOUR_EXTENSION_ID`），先 grep 项目 `.env` 和 `config.ts` 拿真实 URL
...
然后用 `Read` 工具读截图给用户看：标题、正文渲染、配图、链接是否都对。

### 8. 等用户确认后再发布（默认停在预览）

⚠️ **默认不替用户点"创建主题"**。V2EX 主题发出后只有短时间可编辑，且 24 小时内不能删除（积分够才能删）。
...
如果发布失败（节点权限不足、内容触发审核），URL 仍是 `/new/<node>`，页面顶部会有红色错误条。截图回报给用户。
...
`<input name="once">` 是 CSRF token。表单页打开超过几小时后再提交会失败。如果发帖时报"once 过期"，刷新页面重填即可（playwriter 不需要重启）。
...
很多项目 README 里写商店链接但实际是占位（`#` / `YOUR_ID`）。**发帖前 grep `.env` / `config.ts` 拿真链，再 curl 验证 200**。否则发出去就是脸打肿。

### 一帖只能改 10 分钟

V2EX 主题的"编辑"按钮在发布后 10 分钟内可见，之后只能由用户花积分申诉修改。**预览这关一定要让用户亲自看**。
...
playwriter 可能报 `Target page, context or browser has been closed` 或 `No Playwright pages are available`。处理：

1. 调 `mcp__playwriter__reset`（或 `npx playwriter@latest session new`）
2. 如果仍报无可用页 → 让用户在 Chrome 里点 playwriter 扩展图标激活当前 tab，然后再次 reset
3. reset 成功后检查 `state.page.url()`：若不在 `/new/<节点>` 页面，需要重新 `goto` 并**重填标题、正文、Markdown 切换**（之前的输入会全部丢失）
4. 不要静默失败，告诉用户"重新填一次表单"

### 区分"在 v2ex 发帖"与"问 v2ex 的问题"

skill 触发动词必须是"发/提交/发布/post"。如果用户只是问"v2ex 怎么用 markdown / v2ex API 是什么"，**不要**触发本 skill，正常答疑即可。
