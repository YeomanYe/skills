...
- **主体 skill 已经被显式调用**(`exp-sum` / `mem` / `change-recap` / `meta-skill` 等):hat 不挡主流程,只在最终响应末尾追加告知行(详见下方 Relationship to Other Skills "跟其他 meta 类 skill 的优先级"段)
...
| "exp-sum 在跑,我也要把 hat 告知行夹到分诊报告里" | **不行**。告知行只进**对话响应**,不进 exp-sum / mem / change-recap / meta-skill 的结构化产物 |
