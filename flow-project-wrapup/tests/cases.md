...
验证 `flow-project-wrapup` 是否正确触发,并产出预期的三阶段收尾交付。
...
> 我的浏览器扩展 ext-helper 主功能已经写完了,准备做收尾:把代码里的设计同步到设计系统、出一个 README、再做个落地页。

预期:触发本 skill。输出包含 Step 0 项目快照 + Step 1 文档同步明细(注明每类文档状态) + Step 2 README 状态 + Step 3 落地页(因为是浏览器扩展,不是网站) + 风险与开放决策清单。

### T2. 英文 paraphrase

Prompt:

> The CLI we've been building is feature-complete. Time to wrap up: sync code design into our docs, refresh the README, and build a landing page.

预期:触发本 skill。同样产出五节报告。CLI 命中"非网站",触发 Step 3。

### T3. 三选二 + 收尾意图

Prompt:

> 项目要交付了,帮我把 README 出一下,顺便补一下落地页。

预期:触发本 skill(收尾意图明确,两件以上)。Step 1 文档同步若无目标文档,要在报告中显式标"未发现",**不得**默默跳过这一阶段或主动新建占位。
...
预期:不触发本 skill。路由到 `clean-commit`。
...
1. Agent 先做 Step 0 探测,写下 6 个字段(类型 / 前端栈 / 是否网站 / 现存文档 / 设计源 / 路线图源)
...
4. Agent 进入 Step 3,识别"非网站",调 `huashu-design` 要 3 套方向,等用户挑选,再调 `frontend-design` 产出代码;落地页内容三段(Outline/Roadmap/Links)齐全
5. Agent 产出 5 节收尾报告
...
- 不把下游 skill (`huashu-design` / `frontend-design`)的文档原文内嵌进本 skill 正文
- 输出契约与 SKILL.md 声明的结构一致(Step 0 → Step 1 → README → Landing → 风险与开放决策)
