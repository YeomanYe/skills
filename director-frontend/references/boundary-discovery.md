# Boundary Discovery — 焦点向外发现法

> 由 `director-frontend` 的 boundaries / extract mode 使用。
> 原 `ui-extract` skill 的核心方法,合并入 director-frontend。
...
焦点向外发现法只产生**候选**,不自动决定公共组件。

## 最小抽取单元

最小抽取单元不是单个 DOM 标签,而是**具备完整视觉、交互和状态边界的最小 UI 单元**。

候选组件至少满足:

- 有可识别的视觉边界,或明确属于已有 UI primitive
- 有完整交互语义(focus、hover、disabled、loading、error)
- 有稳定状态范围(状态只服务当前候选边界)
- 能用一个清晰名称表达意图
- 抽出后不会让调用方更难读
...
| 元素 | 是否最小抽取单元 |
|---|---|
| 裸 `input` | ❌ 通常不是;`input + border + label + error + help text` 才是 `TextField` 候选 |
| 裸 `button` | ⚠️ 看情况:样式 primitive 可以是 `Button`;带 CTA 文案/业务事件/路由时,应保留在业务组件或调用方 |
| 孤立 `icon` | ❌ 通常不是组件,除非它来自图标系统或有独立交互状态 |
| 只有 margin/padding/flex/grid 的 wrapper | ❌ 不是组件 |
| `Card` | ✅ 可以是 UI primitive |
| `PricingCard` | ✅ 是业务组件(包含价格、套餐和购买动作) |

**最小单元无法被清晰命名 → 不应抽取**;应继续向外寻找更完整的边界,或保留在调用方。
...
- 再向外会把多个变化原因绑在同一个组件里
- 组件名称开始变得模糊(`FlexibleSection` / `MarketingBlock` / `CustomCard`)

## 组件层级 4 层归类

每个候选必须归到下面 4 层之一:

### 1. primitive(基础 UI 组件)

**放置**:低语义、稳定 API、跨项目或跨页面通用的组件。

**适合**:
- `Button` / `Input` / `Card` / `Badge` / `Tabs` / `Dialog`

**不适合**:
- 业务文案 / 业务路由 / 营销图片 / 价格、套餐、注册、支付等业务语义

### 2. shared(公共组件)

**放置**:跨多个页面复用、语义稳定、高于 UI primitive。

**适合**:
- `SectionHeader` / `LogoCloud` / `StatCard` / `FeatureGrid` / `TestimonialCard`

**必须满足**(强制):
- ≥ 2 个页面真实复用证据,或项目中已有相同模式
- props API 不绑定单个页面的业务数据
- 不包含固定文案、固定图片、固定 CTA 路由
- 样式变化可以通过有限、稳定的 variant 表达

**无真实复用证据 → 不进 shared**,保留 page-local 或 business。

### 3. business(业务组件)

**放置**:带业务语义、业务数据结构或业务动作的组件。

**适合**:
- `PricingCard` / `PlanComparison` / `SignupEmailCapture` / `CheckoutCTA` / `ProductFeatureCard`
...
**目录约定**:通常放 `features/<domain>/components/<name>.tsx`。

### 4. page-local(页面局部组件)

**放置**:只服务当前页面叙事或当前页面布局的组件。

**适合**:
- `HeroSection` / `PricingSection` / `TestimonialsSection` / `FinalCTASection`
...
**目录约定**:通常放在页面文件夹下(`pages/<page>/components/` 或 `app/<route>/components/`)。
...
当前边界:
焦点来源:
建议层级: primitive | shared | business | page-local
抽取理由:
不放入更高层级的理由:
建议文件位置:
props API:
...
- 公共组件必须有稳定语义和低业务耦合
- 复用证据不足 → 先放页面局部或业务目录,不要进入 shared
