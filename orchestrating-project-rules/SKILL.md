---
name: orchestrating-project-rules
...
它不替代 `project-rules-architecture`，也不替代任何具体的技术栈 best-practice skill。它的职责是：
...
4. `project-rules-architecture` **永远**纳入
...
记录清单：入口文件、正文文件、重复内容、放错层的内容、明显缺失的领域。这一层跟 `project-rules-architecture` 的盘点口径保持一致。

## Step 5: 联合评估

让每个被选中的 skill 在自己的职责范围内审视当前规范：

- `project-rules-architecture` 负责：入口结构、分域、层级、重复、混层、优先级
...
落地后可建议（不强制）调用 `committing-clean-changes` 做一次收尾提交。若用户明确要求「顺便提交」「帮我 commit」，**不要**自己 `git add` / `git commit`，转交给 `committing-clean-changes` 处理。
...
- `project-rules-architecture` 不在本地：继续，但在最终报告中注明缺失的是结构评估维度
...
- 跳过 `project-rules-architecture`，只靠技术栈 skill 评估
- 保留双轨规则体系（新旧同时有效）
