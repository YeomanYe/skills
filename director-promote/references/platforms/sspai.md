...
并在编辑器/预览就绪后根据用户当前明确确认决定是否点击最终发布。整个流程的关键不在「抢先发」,而在五件事:
...
- 最终发布必须有编辑器/预览就绪后的当前明确确认,否则停在待发布
...
少数派发布是单向、公开、立刻可见的操作(立即发布通道),所以必须满足两层条件:

1. 内容已在编辑器或预览里完成核对。
2. 用户在内容就绪后给出当前明确发布指令,例如"发布吧"、"确认发布少数派"、"少数派也帮我点击吧"。

满足两层条件时,可以替用户点击最终发布按钮。发布后必须读取页面状态,记录公开 URL 或失败原因。
没有当前明确发布指令时,停在待发布状态并说明还缺最后确认。
...
- 想在内容未预览/未确认时自动点「发布」按钮——必须停止;只有编辑器/预览就绪且用户当前明确要求发布时才可点击
...
| Agent 在没有当前明确确认时替用户点了发布按钮 | 停在编辑器、报告状态、等待用户明确发布指令 |
...
参考案例:UniMail 多平台宣发
- 起初规则误把 sspai 设为"永不代点最终发布",导致用户在已确认发布后仍无法委托完成最后一步。
- 修正后的边界:不是"永不点击",而是"内容就绪后必须有当前明确发布指令"。
- 同轮还暴露出多平台发布常见阻塞:V2EX 邮箱验证、Appinn 待处理审核、Product Hunt 需要选择最近 launch 日期。
- 记录要求:每个平台都要区分 `published` / `scheduled` / `submitted-to-review` / `ready-for-confirmation` / `blocked`,不要把"预览正常"当成发布完成。
