...
并行编排通用规范见 `references/parallelization-template.md`；本 skill 专属派工模板（variants 调性派工 prompt、director-design 按平台分化派工 prompt、多平台并行派工表格）见 `references/promote-parallelization.md`。派工 prompt **必须显式声明每路 subagent "必须调用 `director-design` skill"**（subagent 默认不会主动 invoke skill）。

本 skill 的并行集合摘要：
- **variants 模式**（2-3 路，并行）：各调性独立写 `.agent/jobs/promote-variant-N/`，collect-all 汇总；派工 prompt 见 `references/promote-parallelization.md`
- **dispatch 模式**（多平台，**串行不并行**）：每平台都有"等用户确认"步骤，并行会撕裂注意力；按优先级顺序串行，每发完一个出小结再发下一个
- **audit / draft / recap**：单流程串行，不并行
...
通用 schema 见 `references/handoff-payload-template.md`；本 skill 专属接收字段见 `references/promote-handoff-fields.md`。
上游已传的字段本 skill 直接用，不重复探测，不冗余追问。未传时自行探测（Step 1 + Step 3）。
