...
> 本文档是 `exp-sum` 的层级出口字典。每层定义 = **叙事模板** + 性质 + 该放什么 + 不该放什么 + 写在哪里 + 写完后续动作。
>
...
| L9a mem(unblock 分类) | 把**<踩过的坑和绕过去的办法>**变成了**通用错题本案例**,沉淀到了**跨 agent 错题集** | 把"这个坑的解法"变成了通用错题本案例,沉淀到了跨 agent 错题集 |
...
## L9a: `mem/data/unblock/<slug>.md`(跨 agent 卡壳-解法,优先于 L9b)
...
**路径**: `~/Documents/projects/skills/mem/data/unblock/<slug>.md`
...
2. 落盘到 `mem/data/unblock/<slug>.md`
3. **必须同步更新** `mem/INDEX.md` 两处(按 tag 分类 + 按 symptom 关键词反查)
4. commit 到中心 — pre-commit hook 跑 skill-doctor 检查 frontmatter

**唯一合法入口**: mem/data/unblock/ 只能经本 skill Q9a 生成,直接写入会被 mem 拒绝。
...
**不该放什么**:
- 跨 agent 通用工程解法(放 L9a mem unblock 分类)
- 项目特定规则(放 CLAUDE.md)
- 一次性任务上下文
