...
## Failure Mode Catalog(10 个灾难性场景 + detection + recovery)

Red Flag 拦的是"写之前的可见信号";Failure Mode 拦的是"写之后才能观察到的灾难状态"。每个 FM 有 machine-detectable 的 `detect:` 和可执行的 `recovery:`。**任一 FM 命中即触发对应 recovery,不进 Step 5 直到 recovery 完成**。

- **FM-1 跨项目 memory 写串**: 同名 memory 在多项目 `~/.claude/projects/<proj>/memory/` 复用,导致语义在另一个项目被误用。
  - detect: 同 name slug 出现在 ≥ 2 个 project memory/ + frontmatter `type` 不同 / `project:` 字段冲突
  - recovery: 自动加 project-prefix 重命名(`<proj>_<slug>.md`)+ 写迁移 log 到 `MEMORY.md` 末尾 + STOP 等 user 确认重命名

- **FM-2 race condition(并发 session 写同一 MEMORY.md)**: 两个 claude session 同时编辑导致后写覆盖前写。
  - detect: 读 MEMORY.md 时记的 mtime,写盘前再 stat 一次,**mtime 不一致 = 已被并发 session 改过**
  - recovery: 重读最新 MEMORY.md + 用 diff 三方合并(本次新增条目 append 到末尾,不覆盖中间)+ STOP 输出"检测到并发写,已合并,确认 yes 才写盘"

- **FM-3 误沉淀 in-progress task(应该 TaskCreate 而不是 memory)**: 把当前正在做的事写成 long-term memory。
  - detect: memory body 含 `now / today / current / in-progress / TODO / 正在 / 还没` 任一时间词(与 RF1 重合但 FM-3 是"已写完才发现"的兜底)
  - recovery: 撤回该 memory + 自动转 TaskCreate(标题取 body 前 60 字)+ 在 5 段产物的【后续提醒】明示"已转任务,未沉淀 memory"

- **FM-4 reference URL 过期(stale link)**: 2 个月前的 Linear / Jira / Notion URL 现在 404 但 memory 还在 link。
  - detect: `type=reference` 的 memory + frontmatter `last_access > 30d` + body 含 http(s):// URL → 后台 HEAD 探活,non-2xx / DNS 失败 = stale
  - recovery: 给该 memory frontmatter 加 `stale: true` + 在 MEMORY.md index 标红 + 提示 user "URL 已失效,重新提供 / 删除?"

- **FM-5 删 memory 留下 cross-ref 悬空链接**: user 说"忘记 X",但其他 memory 还在 `[[X]]` 引用。
  - detect: 删除 X 后 grep `\[\[X\]\]` 在 `~/.claude/projects/*/memory/*.md` 仍有命中 → 悬空 ≥ 1 处
  - recovery: 批量把 `[[X]]` 改成 `[[~~X~~]]`(strikethrough 标删)+ 在每个引用文件末尾加 `<!-- stale-ref: X removed YYYY-MM-DD -->` + 列清单给 user 确认是否一并清理

- **FM-6 PII 漏写入 memory body(SC3/RF3 漏网)**: 上游 service 返回的 JSON 含 email/token/IP 被原样塞进 body。
  - detect: 写盘前最后一道 regex sweep `\b[\w.+-]+@\w+\.\w+\b` / `sk-[a-zA-Z0-9]{20,}` / `\b(?:\d{1,3}\.){3}\d{1,3}\b` / `Bearer\s+[A-Za-z0-9._-]+` 任一命中
  - recovery: **拒写**(不是 redact 后写)+ 输出"FM-6 PII 命中: `<pattern>`,先 redact 再重跑 SC3" + STOP

- **FM-7 feedback memory 跟当前 user 行为矛盾**: user 上次说"不要 X",这次主动做 X。
  - detect: feedback memory body 关键词 vs 当前 session 最近 20 message diff,出现 `不要 X` / `避免 X` / `禁止 X` 但 session 内 user 主动触发 X
  - recovery: 给旧 feedback memory 加 `superseded: true` + `superseded_at: <date>` + `superseded_by: <session-id>` + 输出"FM-7 矛盾:旧偏好 vs 新行为,确认废弃 / 保留 / 二者并存(条件化)?"

- **FM-8 judgment-tree Q0-Q10 全 no 但 session 含高价值经验**: 罕见但发生过(经验维度不属于任何已有 12 层)。
  - detect: Q0-Q10 走到底全 no + 但 last 10 message 含明显 user 决策动词(`改成 / 决定 / 以后都 / 这次起`)或 user 主动纠正 agent
  - recovery: 兜底走 **reference 类型** + 在【分诊结论】明示"judgment-tree 未命中,推断为 reference,确认 yes 才落盘" + 写入 `MEMORY.md` 时打 tag `fm8-fallback` 供后续 audit

- **FM-9 沉淀路径写错(落盘到根而非 project)**: 该写 `~/.claude/projects/<proj>/memory/<slug>.md` 写成了 `~/.claude/<slug>.md`。
  - detect: 写盘后 `dirname` 跟期望 path 对比,**不一致即 FM-9**;期望 path 由 frontmatter `scope` 字段决定(`scope: project` → projects/<proj>/memory/;`scope: global` → ~/.claude/memory/)
  - recovery: `mv` 到正确路径 + 改 frontmatter `path` 字段 + 在【后续提醒】列出实际落盘 path 供 user 验证

- **FM-10 沉淀触发频率失控(每条 user 消息都沉淀 → 噪音)**: 同 session 1h 内沉淀 ≥ 3 条,信号变噪音。
  - detect: `MEMORY.md` 最近 60 分钟内 append 次数 ≥ 3(看 entry 的 `created_at` frontmatter)
  - recovery: 强制冷却 — 第 4 条起**不新建 memory**,改为 append 到最近一条 memory 的 `addendum:` 字段;若 user 坚持新建,要求显式说"忽略 FM-10 冷却"才放行

**FM 跟 RF / SC 的分工**:
- **SC**(Pre-action)= 起跑线闸,5 道全过才进入 Step 2
- **RF**(Red Flag)= 写盘前的可见信号(body 文本 / 出口选择)
- **FM**(Failure Mode)= 写盘前后才能观察到的状态级灾难(并发 / 链路腐烂 / 路径错位 / 频率失控)

**机器实现提示**: FM-1/4/5/9/10 需要扫盘(grep / stat / mv);FM-2 需要 mtime double-check;FM-3/6/7 是 body / context regex;FM-8 是 judgment-tree 出口兜底。
