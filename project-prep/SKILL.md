...
4. **复用项目真实组件（适用于本身就有 UI 组件的项目）**
   - 如果项目本身就是 component-based frontend——浏览器扩展（popup / options / sidepanel / content script UI）、移动 app 的 web 预览版、独立 widget、嵌入式组件——preview **必须复用项目自己的组件**，不要另起一份只为 preview 存在的 mock 组件。
   - 正确做法（以浏览器扩展为例）：
     - 真实组件路径：`src/popup/Popup.tsx`、`src/options/Options.tsx`
     - Preview 入口：单独的 Vite / Next 配置，**直接 import 这些组件**，外层套一层 mock storage adapter / mock platform API
     - 数据替换发生在**适配器层**（典型如 `isDevMode()` 判断），而不是替换组件本身
     - 状态切换控制器叠加在 preview 外壳，**不污染真实组件代码**
   - 反模式：
     - 另起一份 `_preview/MockPopup.tsx` / `mocks/PopupShell.tsx`，复制粘贴一份长得像但是分叉的实现 → 随时间漂移、双倍维护、走查结果不代表真实组件
     - 把 mock 数据 / 状态切换逻辑**写进真实组件**（用 `if (PREVIEW_MODE)` 污染生产代码）
   - 唯一例外：**项目还在零代码阶段、UI 组件层完全不存在** → 可以临时用占位组件搭壳，但 MVP 第一周内必须切回"真实组件 + 适配器层"模式，不可长期维持双套实现。
   - 适配器层最少要做的事：抽象 storage / platform API / network / auth 这几个外部依赖，让真实组件在 preview 和真实运行面下都能用同一份代码跑。
...
- Component reuse plan: （Required 且项目有 UI 组件时必填：preview 直接复用哪些真实组件、适配器层切在哪、零代码阶段的临时占位边界）
...
| "preview 单独写一份 mock 组件比较快" | 短期快、长期双倍漂移：真实组件改了 mock 不同步，走查就在和"已经过时的 UI"较劲。复用真实组件 + 适配器层切数据，从一开始就是单一真相 |
| "把 mock 数据 / 状态切换 if 写进真实组件就好" | 这是把 preview 的复杂度漏到生产代码里。`if (PREVIEW_MODE)` 的分支会成为长期祖传烂代码。隔离边界放在适配器层 / 外壳层，不要污染组件 |
| "项目还没代码，先用占位组件搭一下 preview 没事" | 仅限 MVP 第一周可接受。一旦真实组件落地必须切回复用模式。占位组件长期保留 = 双套实现的起点 |
...
- 若为 `Required` 且项目本身有 UI 组件（extension / native / widget / 嵌入式），已写明 **Component reuse plan**：复用哪些真实组件、适配器层切在哪、零代码阶段的临时占位边界
